...
const Contact = () => {
  ...
  return (
    ...
    >
      <div className="grid md:grid-cols-2 grid-cols-1 gap-[4rem]">
        ...
        <div className="flex flex-col gap-[4rem]">
          <div className="flex flex-col gap-[1.5rem]">
            ...
            <h1 className="text-">
              Have a project or idea? Let’s connect! Whether it’s developing a
              full-stack web application or sleek designs, I’m here to bring
              your vision to life.
              {/* Got
          ...
          <div className="flex flex-col gap-[1rem] md:gap-[2rem]">
            ...
            {contactIconList.map((contact, index) => (
              <div key={index} className="flex gap-6 items-center">
                ...
                <div>
                  <h5 className="text-lightGrey"> {contact.header} </h5>
                  <p> {contact.text} </p>
                </div>
              </div>
            ))}
        ...
        <div className="flex flex-col gap-[1.5rem]">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-[2rem]">
            {/* name */}
            <div className="col-span-2 lg:col-span-1">
              ...
              />
              {formErrors.name && (
                <p className="text-red-500 text-[0.75rem]">{formErrors.name}</p>
              )}
            </div>
            ...
            <button
              className={`rounded-md py-[15px] px-2 flex items-center justify-center  gap-2 lg:w-[80%] w-full   login-button-glass-background bg-opacity-30  shadow-lg ${
                isValidated ? "bg-[#9d377ebb]" : "bg-[#55535523]"
              }`}
            >
              <p className={`${isValidated ? "text-white" : "text-lightBg"}`}>
                Submit{" "}
              </p>
      ...
      {alert && (
        <div className="absolute z-50 w-1/2 top-3  flex justify-end right-14">
          <Alert
            variant="filled"
            className="w-1/2"
